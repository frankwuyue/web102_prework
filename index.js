...
let sum = GAMES_JSON.reduce( (total, item) => {
...
let unfundedCount = GAMES_JSON.reduce( (total, item) => {
		return item.pledged < item.goal ? total + 1 : total;
}, 0);

// create a string that explains the number of unfunded games using the ternary operator
let unfundedString = unfundedCount === 1 ? "game" : "games";
...
let element = document.createElement("div");
element.innerHTML = `
	<p>A total of $${GAMES_JSON.reduce( (total, item) => {
		return total + item.pledged;
	}, 0).toLocaleString()} has been raised for ${GAMES_JSON.length} games. Currently, ${unfundedCount} ${unfundedString} remains unfunded. We need your help to fund these amazing games!</p>
`;
